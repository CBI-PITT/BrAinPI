...
pyramidLayers = {}
subsets = [2**(len(pyramidMap)-1)]
while subsets[-1] > 2:
    subsets.append(subsets[-1]//2)
...
while startingLocation <= imageStack.shape[0]:
    ...
    for key in pyramidMap:
        ...
        toWrite = fullResChunk[layersToWrite,::2**key,::2**key]
        
        if key == 0:
            advanceNum = toWrite.shape[0]
        start = idx*advanceNum
