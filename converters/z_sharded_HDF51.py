...
class z_sharded_builder:
    ...
    def __init__(
            ...
            sim_jobs=5, compressor=hdf5plugin.Blosc(cname='zstd', clevel=8, shuffle=hdf5plugin.Blosc.BITSHUFFLE),
    ...
    def store_location_formatter(self,res,t,c,z):
        ...
        0,8,16...
        '''
        return os.path.join(self.out_location,'{}/{}/{}/{}.{}'.format(res,t,c,z,self.store_ext))
            ...
            for c in range(self.Channels):
                for key in self.pyramidMap:
                    ...
                    for z_shards in range(0,self.pyramidMap[key][0][0],self.pyramidMap[key][1][0]):
                        ...
                        with h5py.File(location,'a') as store:
                            ...
                            store.create_dataset("data", (z_shape,*self.pyramidMap[key][0][1:]), chunks=(z_shape,*self.pyramidMap[key][1][1:]),dtype=self.dtype, **self.compressor)
