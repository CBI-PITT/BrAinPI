...
# location = r"Z:\zarr"
if os.name == 'nt':
    out_location = r"z:/testData/bil/h5_shard_dask"
else:
    out_location = r"/CBI_FastStore/testData/bil/h5_shard_dask"
...
class z_manager:
    def __init__(self,file,shape,chunks,dtype,compressor):
        self.file = file
        self.shape = shape
        self.chunks = chunks
        ...
        if os.path.exists(file):
            ...
            with h5py.File(file,'r') as f:
                ...
                self.size = f['data'].size
                self.nbytes = f['data'].nbytes
        else:
            ...
            with h5py.File(file,'a') as f:
                ...
                f.create_dataset("data", self.shape, chunks=self.chunks,dtype=self.dtype, **self.compressor)
                self.ndim = f['data'].ndim
                self.size = f['data'].size
                self.nbytes = f['data'].nbytes
    ...
    def __setitem__(self,key,data):
        with h5py.File(self.file, 'a') as f:
            ...
            dset = f['data']
            dset[key] = data
            # dset.write_direct(data,np.s_[:],np.s_[key])
            return 
...
class z_sharded_builder:
    ...
    def __init__(
            ...
            sim_jobs=8, compressor=hdf5plugin.Blosc(cname='zstd', clevel=4, shuffle=hdf5plugin.Blosc.BITSHUFFLE),
        ...
        #Register z_shard properties and z_manager class for each shard
        #Keys are file name for each shard
        self.record_z_shards()
        
        for t in range(self.TimePoints):
            for c in range(self.Channels):
                stack = [da.from_array(
                    self.shards_dict[x]['shard'],chunks=self.shards_dict[x]['shard'].chunks
                    ) for x in self.list_files(0,t,c)]
                print(stack[0],stack[-1])
                stack = da.concatenate(stack,axis=0)
                if c == 0:
                    final_stack_color = [stack]
                else:
                    final_stack_color.append(stack)
            print(final_stack_color)
            
            final_stack_color = da.stack(final_stack_color)
            print(final_stack_color)
            if t == 0:
                final_stack_time = [final_stack_color]
            else:
                final_stack_time.append(final_stack_color)
        
        self.stack = da.stack(final_stack_time)
            ...
            for c in range(self.Channels):
                for key in self.pyramidMap:
                    ...
                    for z_shards in range(0,self.pyramidMap[key][0][0],self.pyramidMap[key][1][0]):
                        ...
                        self.shards_dict[location]['shard'] = z_manager(
                            location,
                            self.shards_dict[location]['shape'],
                            self.shards_dict[location]['chunks'],
                            self.dtype,
                            self.compressor
                            )
    ...
    def write_resolution(self,res,client):
        ...
        for t in range(self.TimePoints):
            for c in range(self.Channels):
                # Create a 2x2 grid in xy to work on follow z-steps
                for y,x in product(range(2),range(2)):
                
                    ylen = math.ceil(parent_shape[-2] / 2)
                    xlen = math.ceil(parent_shape[-1] / 2)
                    
                    ystart = 0 if y == 0 else y * ylen
                    ystop = ystart + ylen if ystart + ylen <= parent_shape[-2] else parent_shape[-2]
                    xstart = 0 if x == 0 else x * xlen
                    xstop = xstart + xlen if xstart + xlen <= parent_shape[-1] else parent_shape[-1]
    ...
    def read_downsamp_write_resolution(self,vds_res,read_slice,downsample_factor,file_to_write,yx_index):
        ...
        data = self.dump_to_zip_force_shape(file_to_write,data,read_slice[3:])
        return True
    
    def dump_to_zip_force_shape(self,file,data,write_slice_yx):
        print('Writing {}'.format(file))
        with h5py.File(file,'a') as store:
            dataset = store["data"]
            # dataset[:] = data[0:dataset.shape[0],0:dataset.shape[1],0:dataset.shape[2]]
            dataset.write_direct(data,np.s_[0:dataset.shape[0],write_slice_yx[0],write_slice_yx[1]],np.s_[:,write_slice_yx[0],write_slice_yx[1]])
        return 'complete'
