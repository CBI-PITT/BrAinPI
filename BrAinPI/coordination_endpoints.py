...
import os, glob
from flask import render_template, request, send_file, redirect, jsonify, url_for
...
from ome_zarr_s3 import get_omezarr_s3_bucket
from utils import (
    from_path_to_html,
    get_path_map,
...
def inititate(app, config):
    ...
    def html_options():
        ...
        if (
            verify_file_exists is None
            or verify_file_exists.lower() == "true"
            or verify_file_exists.lower() == "t"
        ):
        ...
        return jsonify(
            path_to_html_options(
                request.args["path"], verify_file_exists=verify_file_exists
            )
        )
    ...
    def path_to_html_options(path, verify_file_exists=True):
        ...
        path_map = get_path_map(
            settings, user_authenticated=True
        )  # <-- Force user_auth=True to get all possible paths, in this way all ng links will be shareable to anyone
        key = path
        ...
        if html_path is not None:
            ...
            if ng_link is not None:
                ...
                omezarr_entry = paths["neuroglancer"].replace(
                    url_for("neuro_glancer_entry"), url_for("omezarr_entry")
                )
                s3_bucket = get_omezarr_s3_bucket(config)
                omezarr_s3_entry = paths["neuroglancer"].replace(
                    url_for("neuro_glancer_entry"),
                    url_for("omezarr_s3_entry", bucket=s3_bucket) + "/",
                )

                paths["omezarr"] = omezarr_entry + ".ome.zarr"
                paths["omezarr_s3"] = omezarr_s3_entry + ".ome.zarr"
                paths["omezarr_neuroglancer_optimized"] = omezarr_entry + ".ng.ome.zarr"
                paths["omezarr_8bit"] = omezarr_entry + ".8bit.ome.zarr"
                paths["omezarr_8bit_neuroglancer_optimized"] = (
                    omezarr_entry + ".ng.8bit.ome.zarr"
                )
                paths["omezarr_metadata"] = omezarr_entry + ".ome.zarr/.zattrs"
                paths["omezarr_8bit_metadata"] = (
                    omezarr_entry + ".8bit.ome.zarr/.zattrs"
                )
                paths["omezarr_validator"] = validator_url + omezarr_entry + ".ome.zarr"
                paths["omezarr_8bit_validator"] = (
                    validator_url + omezarr_entry + ".8bit.ome.zarr"
                )
                paths["omezarr_neuroglancer_optimized_validator"] = (
                    validator_url + omezarr_entry + ".ng.ome.zarr"
                )
                paths["omezarr_8bit_neuroglancer_optimized_validator"] = (
                    validator_url + omezarr_entry + ".ng.8bit.ome.zarr"
                )
        ...
        for set_name, file in locations.items():
            ...
            with open(file, "r") as f:
                ...
                for line in f.readlines():
                    ...
                    dataset = {
                        "name": strip_trailing_new_line(name),
                        "links": fix_special_characters_in_html(
                            query_to_path_to_html_options
                        ),
                    }
