...
def convert():
    ...
    dataToConvert = "/CBI_Hive/globus/pitt/bil"
    # dataToConvert = "/CBI_Hive/globus/pitt/bil/TEST"
    outLocation = '/CBI_FastStore/zarr'
    # outLocation = None
    ...
    outputLocation = os.path.join(outLocation,'fMOST.zarr')
    ...
    for t,c in itertools.product(
        ...
        for r in resolutions:
            ...
            ## Make sure Z-chunks are a reasonable size - multiple of zarr z-chunk - currently 4
            zChunk = 4
            zChunk = maxChunkSize[0]
            currentChunks = resolutions[r][2]
            if currentChunks[0] < zChunk:
                currentZ = 0
                while currentZ < zChunk:
                    currentZ += currentChunks[0]
                currentChunks = (currentZ,currentChunks[1],currentChunks[2])
                print('New Chunk Value {}'.format(currentChunks))
            ...
            if r == 0:
                ...
                single_color_stack = single_color_stack.rechunk(chunks=(
                    # processingChunks
                    currentChunks
                    ))
                ...
                print('Loading Zarr from disk')
                location=os.path.join(outputLocation,str(r-1).zfill(2))
                single_color_stack = da.from_zarr(location)
                single_color_stack = single_color_stack[t,c]
                ...
                if a.chunksize != currentChunks:
                    a = a.rechunk(currentChunks)
            ...
            print('Processing Resolution {}, TimePoint {}, Color {}'.format(r,t,c))
            
            batchNumber = 60
            batch = []
            # print(make.shape)
            for ii in range(make.shape[0]):
                # print(ii)
                
                print('Calling Compute on layer {} of {}'.format(ii,make.shape[0]))
                # z = client.compute(make[start:stop])
                run = make[ii:ii+1]
                run = run.sum()
                ...
                if len(batch) < batchNumber:
                    # print('Waiting to submit next layer')
                    time.sleep(1)
                else:
                    while batch[-batchNumber].status != 'finished':
                        time.sleep(1)

                # print('Waiting for Compute to finsh')
                # z = client.gather(z)
    ...
    #             # if single_color_stack.chunksize != processingChunks:
    #             #     single_color_stack = single_color_stack.rechunk(processingChunks)
    ...
    #     toMake = []
    #     for r in resolutions:
    #         print('Setup save for resolution {}'.format(r))
    #         location=os.path.join(outputLocation,str(r).zfill(2))
    #         make = downSample[r]
    #         currentChunks = resolutions[r][2]
    #         if make.chunksize != currentChunks:
    #             if currentChunks[0] < 10:
    #                 currentChunks = (10,currentChunks[1],currentChunks[2])
    #             make = make.rechunk(currentChunks)
    #         make = da.map_blocks(saveToZarr,
    #                       make,
    #                       t,
    #                       c,
    ...
    #     z = client.compute(toMake[0:2])
    #     # z = client.compute(toMake)
    #     print('Waiting for compute to complete')
    #     z = client.gather(z)
